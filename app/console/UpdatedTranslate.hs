...
module UpdatedTranslate where

import           Data.List                 (find, subsequences)
import           Def
import           FunConversion.OCamlPretty (prettyString)
import           FunConversion.Updated.Syntax      (embedProgSafe)
import           FunConversion.Updated.Trans       (translateMultiMode, translateSingleMode)
...
runWithParser parser inputFile outDir relName inputs = do
  ...
  case program of
    Right program -> do
      ...
      case inputs of
        Just inputs -> do
          let translatedProgram = translateSingleMode relName inputs program
        ...
        Nothing -> do
          ...
          case def of
            Just def -> do
              ...
              let translatedProgram = translateMultiMode (getDefs program) (map (relName,) inputs)
              outputProgram ocamlFile haskellFile uBaseName translatedProgram
