...
import Simple      (Term (..), nthOptIII, nthOptOIO, nthOptOII, nthOptIIO)
...
ans1 = (eval5 (takeS 1) nthOptOIOffline) (rightAns1, natGen, natGen, natGen, natGen)
ans2 = (eval4 (takeS 2) nthOptOII) (n, rightAns1, natGen, natGen)
ans3 = (eval4 (takeS 2) nthOptOII) (n, rightAns2, natGen, natGen)
ans4 = (eval3 (takeS 3) nthOptOIO) (n, natGen, natGen)
...
main = defaultMain
  [
    bgroup "nthOpt"
     ...
     , bench "simpleII"     $ nf (eval3 (takeS 1) nthOptIII)      (rightList1, n, rightAns1)
     ...
     , bench "simpleIO"     $ nf (eval2 (takeS 1) nthOptIIO)      (rightList1, n)
     -- nthOptOIOffline, nthOptOIOnline
     , bench "offlineOI1"    $ nf (eval5 (takeS 1) nthOptOIOffline) $ traceShow ans1 (rightAns1, natGen, natGen, natGen, natGen)
--     , bench "onlineOI1"     $ nf (eval5 (takeS 1) nthOptOIOnline) (rightAns1, natGen, natGen, natGen, natGen) -- doesn't fit in time -- interesting
     , bench "simpleOI1"     $ nf (eval4 (takeS 1) nthOptOII)       $ traceShow ans2 (n, rightAns1, natGen, natGen)
     ...
     , bench "simpleOI2"     $ nf (eval4 (takeS 1) nthOptOII)       $ traceShow ans3 (n, rightAns2, natGen, natGen)
     -- nthOptOOOffline, nthOptOOOnline 5
     , bench "offlineOO"     $ nf (eval5 (takeS 5) nthOptOOOffline) (natGen, natGen, natGen, natGen, natGen)
  --   , bench "onlineOO"      $ nf (eval5 (takeS 1) nthOptOOOnline) (natGen, natGen, natGen, natGen, natGen) -- doesn't fit in time
     , bench "simpleOO"      $ nf (eval3 (takeS 5) nthOptOIO)       $ traceShow ans4 (n, natGen, natGen)
     ]
  ]
