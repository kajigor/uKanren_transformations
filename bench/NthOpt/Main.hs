...
main = defaultMain
  [
    bgroup "nthOpt"
     ...
     , bench "onlineOI1"     $ nf (eval5 (takeS 1) NthOpt_unfold.nthOptOI) (rightAns1, natGen, natGen, natGen, natGen) -- doesn't fit in time -- interesting
     , bench "simpleOI1"     $ nf (eval4 (takeS 1) Simple.nthOptOII)       $ traceShow ans2 (n, rightAns1, natGen, natGen)
--     , bench "offlineOI2"    $ nf (eval5 (takeS 1) NthOpt_unfold.nthOptOI) (rightAns2, natGen, natGen, natGen, natGen)
     ...
     , bench "onlineOO"      $ nf (eval5 (takeS 1) NthOpt_cpd_ans.nthOptOO) (natGen, natGen, natGen, natGen, natGen) -- doesn't fit in time
