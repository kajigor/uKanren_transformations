...
let run_lists n textRepr r =
  Printf.printf "-----------------------------\n%s\n" textRepr;
  List.iter (fun (xs, ys, zs) -> Printf.printf "X: %s\nY: %s\nZ: %s\n\n" (list_to_string xs) (list_to_string ys) (list_to_string zs))  @@
            RStream.take ~n:n @@ r (fun xs ys zs -> (xs, ys, zs))

let reify_triple t = t#reify (Std.Pair.reify (Std.Pair.reify (Std.List.reify Std.Nat.reify) (Std.List.reify Std.Nat.reify)) (Std.List.reify Std.Nat.reify))

let show_triple t = show(Std.Pair.logic) (show(Std.Pair.logic) (show(Std.List.logic) (show(Std.Nat.logic))) (show(Std.List.logic) (show(Std.Nat.logic)))) (show(Std.List.logic) (show(Std.Nat.logic))) @@ t

let triple_to_string t = show_triple (reify_triple t)

let run_list_triple n textRepr r =
  Printf.printf "-----------------------------\n%s\n" textRepr;
  List.iter (fun x -> Printf.printf "%s" (triple_to_string x))  @@
            RStream.take ~n:n @@ r (fun x -> x)
