...
\db{
  ...
  The result of driving is so-called \emph{process tree} where nodes correspond to \emph{configurations} presenting computation state, for example, a term in case of pure functional programming languages.
  ...
  If the whistle signalled during some branch construction, then something is done to ensure termination.
  ...
  One example of the source of infinite branches is consecutive recursive call to the same function with an accumulating parameter: by unfolding such a call further we can only increase the term size and lead to nontermination.
