  ...
  where
    ...
    go definitions node@(Node descend gen _ _ ch) = do
      ...
      maybeToEither
        ...
        (disj disjs)
    go definitions node@(Transient descend ch _ _) =
      nodeToBody definitions ch
    go _ node =
    ...
    nodeToBody _ (Success subst) =
      if Subst.null subst
        then Right success
        else Right $ residualizeSubst subst
    ...
    nodeToBody definitions node@(Node descend gen subst _ ch) =
      maybeToEither
        ...
        (conj $ residualizeSubstConj subst ++ residualizeSubstConj gen ++ [generateInvocation (Descend.getCurr descend) definitions])
    nodeToBody definitions node@(Transient descend ch subst _) = do
      goal <- nodeToBody definitions ch
      maybeToEither
        (printf "Failed to generate a goal from a transient node\n  %s" (show node))
        (conj $ residualizeSubstConj subst ++ [goal])
    nodeToBody _ prune =
...
residualizeGlobalTree tree =
  let nodes = getNodes tree
   in let definitions = foldl (\defs gs -> fst3 (renameGoals gs defs)) [] $ map fst nodes
       in -- trace "Residualize Global Tree" $ trace (intercalate "\n\n" $ map show definitions) $
            ...
            filter isJust $
              map (divideInvocations defs) $
                concatMap (generateSplits goals) $
                  reverse [1 .. length goals]
       in case representable of
            (x : _) -> x
            _ -> Nothing
...
renameGoals gs definitions =
  let ns =
        map
          ...
          gs
   in let actualName = newName (map fst ns) definitions
       in let args = uniqueArgs $ map snd ns
           in let newDefs = (gs, actualName, args) : definitions
               in (newDefs, actualName, args)
...
residualizeSldTree rootGoals tree definitions = do
  let (_, defName, rootVars) =
        fromMaybe (error (printf "Residualization failed: no definition found for\n%s\nDefs:\n%s\n" (show rootGoals) (show definitions))) $
  ...
  if null goals
    then fail (printf "No resultants in the sld tree for %s" (show rootGoals))
      ...
      return $
        ...
        let res = Def defName defArgs body
         in -- traceShow res
            res
  where
    ...
    go subst gs defs =
      let goal = residualizeGoals gs defs
       in unsafeConj $ residualizeSubstConj subst ++ [goal]
