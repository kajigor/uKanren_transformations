...
module Test.Shallow.ShallowToConversion where
...
import Shallow.Interpreters.Typeless

$(return $ S.embedProg "addo" $ TR.transMultiMode (map ((\v -> "x" ++ show v) <$>) (toTypeless $ addo freeArg freeArg freeArg)) [("addo", x) | x <- subsequences [0..2]])

genNat :: Stream Term
genNat = return Z `mplus` (S <$> genNat)
...
unit_ShallowTestAddoGen :: IO ()
unit_ShallowTestAddoGen =
  runTestFtb 50
