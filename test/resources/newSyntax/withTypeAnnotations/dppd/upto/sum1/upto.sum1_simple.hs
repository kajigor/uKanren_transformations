...
data Term
    ...
    | S Term
    deriving (Show, Eq)
sumsquaresuptoIO x0 = msum [do {x2 <- uptoIO x0;
                                x3 <- squaresIO x2;
                                x1 <- sumIO x3;
                                return x1}]
...
multiplyIIO x0 x1 = msum [do {let {x2 = O};
                          ...
                          do {x3 <- case x1 of
                              ...
                              x2 <- addIIO x0 x4;
                              return x2}]
addIIO x0 x1 = msum [do {guard (x0 == O);
                         let {x2 = x1};
                         return x2},
                     do {x3 <- case x0 of
                               {S y3 -> return y3; _ -> mzero};
                         ...
                         let {x2 = S x4};
                         return x2}]
sumIO x0 = msum [do {let {x9 = O}; x1 <- sum1IIO x0 x9; return x1}]
sum1IIO x0 x1 = msum [do {guard (x0 == Nil);
                          let {x2 = x1};
                          return x2},
                      do {(x3, x4) <- case x0 of
                          ...
                          x2 <- sum1IIO x4 x5;
                          return x2}]
uptoIO x0 = msum [do {let {x1 = Nil}; guard (x0 == O); return x1},
                  do {x3 <- case x0 of
                            {S y3 -> return y3; _ -> mzero};
                      let {x6 = x0};
                      x2 <- uptoIO x3;
