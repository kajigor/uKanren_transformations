...
filter (static static)
 gt x y = ((fresh z in ((x == C Succ z & y == Zero))) | (fresh x', y' in ((x == C Succ x' & y == C Succ y' & Unfold gt x' y'))));
filter (static static)
 le x y = (x == Zero | (fresh x', y' in ((x == C Succ x' & y == C Succ y' & Unfold le x' y'))));
filter ()
 fail  = Memo fail [];

(fresh lst, reslst in (Memo sorto lst reslst))
