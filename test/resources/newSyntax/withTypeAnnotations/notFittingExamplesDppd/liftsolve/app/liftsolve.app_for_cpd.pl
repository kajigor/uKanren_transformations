...
? solve([
      term(clause, [term(app,[term(null, []), var(zero), var(zero)])]),
      term(clause, [term(app, [term(cons, [var(zero), var(succ(zero))]), var(succ(succ(zero))), term(cons, [var(zero), var(succ(succ(succ(zero))))])]), term(app, [var(succ(zero)), var(succ(succ(zero))), var(succ(succ(succ(zero))))])])
      ],
      [term(app, [X, Y, Z])])