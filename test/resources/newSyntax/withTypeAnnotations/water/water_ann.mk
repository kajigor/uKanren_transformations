filter (static dynamic static static)
 checkAnswer' state0 answer reqLvl result =
  ...
  ((fresh x, xs, isStepValid in
    ((Unfold checkStep state0 x isStepValid &
      ((fresh state' in
        ((isStepValid == Trueo &
        ...
        ((t == Pour &
          ...
          ((fresh diff in
            ((isGreater == Trueo &
              ...
              Unfold createState b diff cap2 state))) |
           (isGreater == Falso &
...
filter (static dynamic dynamic)
 checkStep state0 step0 isStepValid =
   (fresh f, s, t, b, lvl1, lvl2 in
    ((state0 == Pair f s &
     ((b == Fst & f == lvl1) | (b == Snd & s == lvl1)) & ((b == Fst & s == lvl2) | (b == Snd & f == lvl2)) & ((t == Fill & Unfold eqNat lvl1 Zero isStepValid) | (fresh q44 in ((t == Empt & Unfold capacities b q44 & Unfold eqNat lvl1 q44 isStepValid))) | (fresh b', q47, lvl1IsO, lvl2IsCap, b'Cap in ((t == Pour & Unfold anotherBottle b b' & Unfold capacities b' b'Cap & ((lvl1IsO == Trueo & q47 == Trueo) | (lvl1IsO == Falso & q47 == lvl2IsCap)) & Unfold eqNat lvl1 Zero lvl1IsO & ((q47 == Trueo & isStepValid == Falso) | (q47 == Falso & isStepValid == Trueo)) & Unfold eqNat lvl2 b'Cap lvl2IsCap)))) & step0 == Pair t b)));
...
filter (static static dynamic)
 add a b r = ((a == Zero & b == r) | (fresh x in ((a == Succ x & Unfold add x (Succ b) r))));
filter (dynamic static static)
